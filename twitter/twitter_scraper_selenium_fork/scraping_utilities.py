...
class Scraping_utilities:
    ...
    def url_generator(keyword: str, since: Union[int, None] = None, until: Union[str, None] = None,
                      ...
                      extra_parameters: dict[str, Union[str, int]] = None) -> str:
        ...
        Args:
            ...
            extra_parameters: (dict[str, Union[str, int]], optional): extra parameters to pass to the query in the format name: value in a dictionary
        ...
        if within_time is None:
            words = [Scraping_utilities.set_value_or_none(since, "since:"),
                     Scraping_utilities.set_value_or_none(until, "until:"),
                     Scraping_utilities.set_value_or_none(since_id, "since_id:"),
                     Scraping_utilities.set_value_or_none(max_id, "max_id:")]
            query = ""
            if extra_parameters is not None:
                for name_p, value in extra_parameters.items():
                    words.append(f"{name_p}:{value} ")
            for word in words:
                if word is not None:
                    query += word
            query += keyword
    ...
    def build_topic_params(rest_id, cursor):
        variables = {"rest_id": rest_id, "context": "{}", "withSuperFollowsUserFields": True,
